...
export class Problems {
  ...
  public static createProblemsFromResults(results: Array<TestResult>) {
    ...
    return problems.reduce((groups, item) => {
      ...
      groups[val].push(
        new vscode.Diagnostic(new vscode.Range(item.lineNumber - 1, 0, item.lineNumber - 1, 10000), item.message)
      );
      return groups;
    }, {});
  }
  ...
  private addTestResults(results: ITestResult) {
    ...
    const newDiagnostics: Array<[vscode.Uri, Array<vscode.Diagnostic>]> = [];
