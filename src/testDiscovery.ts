...
export function discoverTests(testDirectoryPath: string, dotnetTestOptions: string): Promise<IDiscoverTestsResult> {
  return executeDotnetTest(testDirectoryPath, dotnetTestOptions).then(stdout => {
...
function extractTestNames(testCommandStdout: string): Array<string> {
  return (
    testCommandStdout
      .split(/[\r\n]+/g)
      /*
...
function isMissingFqNames(testNames: Array<string>): boolean {
  return testNames.some(name => !name.includes('.'));
}

function discoverTestsWithVstest(assemblyPaths: Array<string>, testDirectoryPath: string): Promise<Array<string>> {
...
function readVstestTestNames(testOutputFilePath: string): Promise<Array<string>> {
  return new Promise((resolve, reject) => {
...
function executeDotnetVstest(
  assemblyPaths: Array<string>,
  listTestsTargetPath: string,
