...
export class Executor {
         private static readonly terminals: { [id: string]: vscode.Terminal } = {};
         private static readonly isWindows: boolean = platform() === 'win32';
         private static readonly onTestProcessFinishedEmitter = new EventEmitter<void>();
         private static debugRunnerInfo?: IDebugRunnerInfo;
         private static processes: Array<ChildProcess> = [];

         public static runInTerminal(
           command: string,
           cwd?: string,
           addNewLine: boolean = true,
           terminal: string = 'Test Explorer'
         ): void {
           ...
           if (addToProcessList) {
             ...
             childProcess.stdout.on('data', buf => {
               ...
               if (this.debugRunnerInfo.config) {
                 ...
                 vscode.debug
                   .startDebugging(vscode.workspace.workspaceFolders![0], this.debugRunnerInfo.config)
                   .then(c => {
                     // When we attach to the debugger it seems to be stuck before loading the actual target assembly
                     // This is to try to continue past this invisible break point
                     setTimeout(() => {
             ...
             childProcess.on('close', () => {
               ...
               this.debugRunnerInfo = undefined;
