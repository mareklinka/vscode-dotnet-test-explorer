...
export class Utility {
  ...
  public static getFqnTestName(testName: string): string {
    // Converts a test name to a fqn version. For instance
    // MyNameSpace.Class("Nunit fixture").TestName("With some arguments here") => MyNameSpace.Class.TestName
  ...
  public static updateCache() {
    ...
    // Cross Mark
    Utility.failed = Utility.getLensText(configuration, 'codeLensFailed', '\u274c');

    // White Heavy Check Mark / Heavy Check Mark
    Utility.passed = Utility.getLensText(configuration, 'codeLensPassed', osx ? '\u2705' : '\u2714');

    // Warning
    Utility.skipped = Utility.getLensText(configuration, 'codeLensSkipped', '\u26a0');
