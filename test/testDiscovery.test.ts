...
suite('Test discovery', () => {
  ...
  setup(() => {
    ...
    fsMkdtempSyncStub.withArgs(path.join(tmpDir, 'test-explorer-discover-')).returns(vsTestOutputTempDir);
  });
  ...
  test('Fully qualified test names returned when dotnet test outputs FQ test names', () => {
    ...
    execStub
      .withArgs(dotnetTestExecCmd, sinon.match.func, testDirectoryPath)
      .callsArgWith(1, undefined, buildDotnetTestOutput(testNames, assemblyFilePath), '');

    return discoverTests(testDirectoryPath, dotnetTestOptions).then(result =>
  ...
  test('Promise rejected when dotnet test failing', () => {
    ...
    execStub.withArgs(dotnetTestExecCmd, sinon.match.func, testDirectoryPath).callsArgWithAsync(1, error);

    return discoverTests(testDirectoryPath, dotnetTestOptions)
      .then(r => {
        throw new Error('Promise was unexpectedly fulfilled. Result: ' + r);
      })
  ...
  test('Empty list returned when dotnet test outputs empty list', () => {
    execStub
      .withArgs(dotnetTestExecCmd, sinon.match.func, testDirectoryPath)
      .callsArgWithAsync(1, undefined, buildDotnetTestOutput([], assemblyFilePath), '');

    return discoverTests(testDirectoryPath, dotnetTestOptions).then(result => assert.deepEqual(result.testNames, []));
  });
  ...
  test('Fully qualified test names returned using dotnet vstest when dotnet test output is missing fq names', () => {
    ...
    fsReadFileStub
      .withArgs(vsTestOutputFilePath, 'utf8', sinon.match.func)
      .callsArgWithAsync(2, undefined, fqTestNames.join('\r\n'));

    execStub
      .withArgs(dotnetTestExecCmd, sinon.match.func, testDirectoryPath)
      .callsArgWithAsync(1, undefined, buildDotnetTestOutput(testNames, assemblyFilePath), '');

    execStub.withArgs(dotnetVstestExecCmd, sinon.match.func, testDirectoryPath).callsArgWithAsync(1, undefined, '');

    return discoverTests(testDirectoryPath, dotnetTestOptions)
    .then(result =>
      assert.deepEqual(result.testNames, fqTestNames)
    );
  });
  ...
  test('Promise rejected when dotnet vstest output file read operation fails', () => {
    ...
    fsReadFileStub.withArgs(vsTestOutputFilePath, 'utf8', sinon.match.func).callsArgWithAsync(2, error);

    execStub
      .withArgs(dotnetTestExecCmd, sinon.match.func, testDirectoryPath)
      .callsArgWithAsync(1, undefined, buildDotnetTestOutput(testNames, assemblyFilePath), '');

    execStub.withArgs(dotnetVstestExecCmd, sinon.match.func, testDirectoryPath).callsArgWithAsync(1, null, '');
  ...
  test('Promise rejected when dotnet vstest returns error', () => {
    ...
    execStub.withArgs(dotnetVstestExecCmd, sinon.match.func, testDirectoryPath).callsArgWithAsync(1, error, '');
  ...
  test('Promise rejected when failed to extract test assembly file name from dotnet test output', () => {
    ...
    execStub
      .withArgs(dotnetTestExecCmd, sinon.match.func, testDirectoryPath)
      .callsArgWithAsync(1, undefined, dotnetTestOutput, '');
  ...
  test('Remove vstest output file and directory after vstest execution', () => {
    ...
    fsReadFileStub.withArgs(vsTestOutputFilePath, 'utf8', sinon.match.func).callsArgWithAsync(2, undefined, '');
    ...
    return discoverTests(testDirectoryPath, dotnetTestOptions).then(r => {
  ...
  test('Solution with multiple test projects fully qualified test names returned using dotnet vstest when dotnet test output is missing fq names', () => {
    const testAssemblyFilePaths = ['/bin/TestAssembly1.dll', '/bin/TestAssmebly2.dll'];

    const testNames = ['Test1', 'Test2', 'Test3', 'Test4'];
    const fqTestNames = ['Ns1.Class1.Test1', 'Ns1.Class1.Test2', 'Ns2.Class2.Test3', 'Ns2.Class2.Test4'];
    ...
    execStub
      .withArgs(dotnetTestExecCmd, sinon.match.func, testDirectoryPath)
      .callsArgWithAsync(1, undefined, buildDotnetTestSolutionOutput(testNames, testAssemblyFilePaths), '');

    execStub
      .withArgs(dotnetVstestMultiProjectExecCmd, sinon.match.func, testDirectoryPath)
      .callsArgWithAsync(1, undefined, '');
  ...
  test('Dotnet test results returned with a warning message when dotnet vstest /ListFullyQualifiedTests switch is not supported', () => {
    ...
    execStub
      .withArgs(dotnetVstestExecCmd, sinon.match.func, testDirectoryPath)
      .callsArgWithAsync(1, new Error(), '', getDotnetVstestListFqnFlagMissingErrorOutput());

    return discoverTests(testDirectoryPath, dotnetTestOptions).then(result => {
      assert.deepEqual(result.testNames, testNames);
      assert.deepEqual(result.warningMessage, {
...
function buildDotnetTestOutput(testNames: Array<string>, testAssemblyFilePath: string) {
...
function buildDotnetTestSolutionOutput(testNames: Array<string>, testAssemblyFilePaths: Array<string>) {
  let output = new Array<string>(testAssemblyFilePaths.length).fill('Build started, please wait...').join('\r\n');
...
function buildDotnetVstestCommand(testAssemblyFilePaths: Array<string>, vstestOutputFilePath: string) {
